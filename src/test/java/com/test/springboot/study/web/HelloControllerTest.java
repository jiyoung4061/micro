...
1.6 HelloController의 테스트모듈 만들때는 뒤에 Test라는 이름을 갖는다.
   ...
   메소드 체이닝(Method Chaining) : mvc.perform().andExpect(status)
    ...
    단위 테스트는 동작이 정상인지만 확인
    실제 서비스를 돌릴려면 Application을 실행 후
...
public class HelloControllerTest {
    ...
    /*
        jsonPath()
            JSON 응답값을 필드별로 검증하느 ㄴ메소드
            $단위로 변수를 구분하고, 앞에 Dot(.)을 써준다. ex) $.name

            Application 을 실행(전체실행)
            주소창에
            http://localhost:8080/hello/dto?name=이순신&age=23
    */
